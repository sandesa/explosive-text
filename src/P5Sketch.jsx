...
export default function P5Sketch() {
    ...
    useEffect(() => {
        let sketch = (p) => {
            let chars = []
            const text = "EXPLODE TEST EXPLODE TEST EXPLODE TEST EXPLODE TEST EXPLODE TEST"
            const fontSize = 80
                ...
                update(mouseX, mouseY, mouseDown) {
                    if (mouseDown) {
                        ...
                        if (distance < explosionRadius && distance > 0) {
                            // Repulsion force
                            const force = (explosionRadius - distance) / explosionRadius * repulsionForce
                    ...
                    // Jittering control
                    if (Math.abs(this.x - this.originalX) < jitterThreshold && Math.abs(this.y - this.originalY) < jitterThreshold) {
